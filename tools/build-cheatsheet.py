...
перед ним: рука веером в левом нижнем углу, сердца аркой сбоку от
персонажа, очки полувеером у его нижнего левого угла, персонаж с бутылкой
яда и роль, над ними ловушка с фигуркой по центру; стойка вровень с ролью,
над ней аура со знаменем по центру; два эффекта внахлёст. Карты — превью из
rules/media/cards/ (для EN — *-en.webp), фигурки и жетоны из rules/media/fig/
(hearts-arc.webp и vp-fan.webp — /img по media/hp.png и winpoint.png). У предметов ничего не рисуется: подпись со значком
просто стоит рядом.
...
fig('hp', 49, 96.5, 15.5, 24, 'media/fig/hearts-arc.webp', cls='tok')   # четыре сердца аркой сбоку от персонажа (/img по media/hp.png)
...
fig('vp', 55, 116, 14, 13, 'media/fig/vp-fan.webp', cls='tok')          # четыре очка полувеером у нижнего левого угла персонажа (/img по media/winpoint.png)
lcard('trapcard', 69, 75, 'back')                  # ловушка — рубашкой вверх, над персонажем
fig('trap', 77.5, 73, 11, 11, 'media/fig/trap.webp')   # фигурка — по центру карты, чуть выше её верха
card('stance', 106, 95, 1166)                     # Лучник — верх вровень с ролью
card('aura',   106, 62, 1206)                      # Дымовая завеса — над стойкой
fig('banner', 111.2, 70, 6.6, 11.2, 'media/fig/banner.webp')   # знамя ауры — по центру карты ауры
...
CALL = [
 ...
 (['hp','char','role'], (5, 70, 51), 'left', 'hp', 'char'),
 (['hand0','hand1','hand2','hand3','hand4'], (5, 124, 48), 'bottom', 'card', 'hand'),
 (['vp'], (56, 130, 27), 'bottom', 'winpoint', 'vp'),    # под очками, по ширине персонажа
 (['poison'], (84, 130, 21), 'bottom', 'poison', 'poison'),                  # под ролью, по её ширине
 (['stance'], (106, 127, 22), 'bottom', 'stance', 'stance'),                 # под стойкой
]
CHAIN_KEYS = ['character', 'stance', 'aura', 'poison', 'effect', 'weapon', 'modifier']   # цепочка порядка расчёта с памятки убрана (chain_html не вызывается)
...
SCRIPT = r"""
  (function () {
    var M = '../media/';
    var GROUP = { defense:'#dca300', trap:'#43525A', weapon:'#2E2A28', stance:'#A78B6B', modifier:'#ED1C24', aoe:'#2F3566',
      aura:'#93a32e', effect:'#5B4A7E', intervention:'#3B8476', action:'#2E2A28', character:'#6C8CC7', role:'#5d3c75' };
