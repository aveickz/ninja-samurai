...
кромки предмета и короткая подпись со значком рядом (без соединительных штрихов).
...
for i in range(5):
    fig(f'hand{i}', 20 + i * 5.2, 36 + (i - 2) ** 2 * .7, 14, 22.4, 'media/cards/back.webp', rot=-30 + i * 7, cls='card hand')
...
CALL = [
 ...
 (['vp0','vp1','vp2','vp3'], (89, 129, 42), 'right', 'Победные очки', 'winpoint',
...
def svg_html():
    ...
    return (f'<svg class="ov" viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg">'
            ...
            '<g class="st" filter="url(#ink)">' + divs + '</g>'      # штрихи от скобок к подписям убраны — только скобки
            '<g>' + ''.join(dots) + '</g></svg>')
