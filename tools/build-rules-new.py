...
по центру; при нечётном числе страниц текста перед ней добавляется пустая.
...
# завершающий лист «Конец» (тоже один по центру; если страниц текста выходит
# нечётное число, перед ним вставляется пустая — чтобы развороты сошлись);
...
def build(lang):
    ...
    # «Конец» стоит один, как титул; страницы текста должны сложиться в целые
    # развороты — при нечётном счёте перед «Концом» вставляется пустая
    # страница (предпоследняя, без фолио).
    pages = list(PAGES)
