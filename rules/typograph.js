/* ============================================================
   ...
   пробел после любого слова в одну-две буквы (предлоги, союзы,
   частицы — по-русски и по-английски) на неразрывный (U+00A0),
   а пробел перед тире «—» — тоже на неразрывный. Так короткие
   слова не «висят» в конце строки, а тире не начинает строку.
   Правило то же, что у описаний карт в картотеке (js/app.js).
...
(function () {
  ...
  // Как в описании карт (js/app.js, typographHtml): любое слово в одну-две буквы,
  // русское или латинское, приклеивается к следующему — список выше остался
  // для справки, правило шире него.
  var PATTERN = new RegExp(
    '(?<![\\p{L}\\p{N}])(\\p{L}{1,2}) (?=[\\p{L}\\p{N}«„(])',
    'gu'
  );
  // Тире не начинает строку: пробел перед «—» — неразрывный.
  var DASH = / — /g;
  var DASH_REPLACEMENT = '\u00A0— ';
  ...
  function process(root) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      var after = before.replace(PATTERN, NBSP_REPLACEMENT).replace(DASH, DASH_REPLACEMENT);
