...
Тиры:
  proto   - sol/high, 688x976   (раскладка 2x2 на четыре варианта в ОДНОЙ картинке)
  preview - luna/low, 704x1008  (один вариант = одно списание, устаревший путь)
  final   - sol/high, 1792x2560 (те же 7:10, под печать)

Прототипирование идёт тиром proto: спек из ОДНОГО элемента, промпт которого
описывает лист 2x2 с четырьмя разными сценами. Это одно списание квоты вместо
четырёх, поэтому агент берётся лучший, а не дешёвый.
...
TIERS = {
    ...
    # proto - одна картинка-раскладка на 2-4 варианта: списание одно, поэтому
    # агент берётся лучший. 688x976 - минимальная легальная площадь при 7:10
    # (655360 px, стороны кратны 16); модель этот размер всё равно игнорирует.
    "proto": ("sol", "high", "688x976", 1200),
